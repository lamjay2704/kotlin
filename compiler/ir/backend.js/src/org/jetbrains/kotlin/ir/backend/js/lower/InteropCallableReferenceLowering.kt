...
class InteropCallableReferenceLowering(val context: JsIrBackendContext) : BodyLoweringPass {
    ...
    override fun lower(irFile: IrFile) {
        ...
        // Regular contextless lambdas are transformed to function references
        val ctorToFreeFunctionMap = mutableMapOf<IrConstructorSymbol, IrSimpleFunctionSymbol>()

        // Regular lambdas with captured variables are transformed to function expressions
        ...
        val capturedVariablesDeclaredInLoops = CapturedVariablesDeclaredInLoops()

        irFile.acceptChildrenVoid(capturedVariablesDeclaredInLoops) // TODO: Only do this if lambda inlining is enabled with a feature flag!

        val callableReferenceClassTransformer = CallableReferenceClassTransformer(
            ctorToFactoryMap,
            ctorToFreeFunctionMap,
            ctorToFunctionExpressionMap,
            capturedVariablesDeclaredInLoops
        )
    ...
    /**
     * This class helps determine if a lambda captures a variable declared in a loop.
     ...
     */
    private class CapturedVariablesDeclaredInLoops : IrElementVisitorVoid {
        ...
        private fun constructorCallHasVariablesDeclaredInLoops(ctorCall: IrConstructorCall) =
            (0 until ctorCall.valueArgumentsCount).any { i ->
                when (val arg = ctorCall.getValueArgument(i)) {
                    is IrGetValue -> variablesDeclaredInLoops.contains(arg.symbol.owner)
                    else -> false
                }
            }
        ...
        fun lambdaCapturesVariablesDeclaredInLoops(lambdaClass: IrClass): Boolean {
            ...
            return ctorCalls.any(this::constructorCallHasVariablesDeclaredInLoops)
        }
    }
    ...
    private inner class CallableReferenceClassTransformer(
        ...
        private val capturedVariablesDeclaredInLoops: CapturedVariablesDeclaredInLoops
        ...
        private fun replaceWithFactory(lambdaClass: IrClass): List<IrDeclaration> {
            ...
            return if (lambdaClass.origin == CallableReferenceLowering.Companion.LAMBDA_IMPL) {
                if (// Suspend lambdas cannot be lowered to in-line anonymous functions
                    lambdaInfo.isSuspendLambda

                    // Previous lowerings like JsSuspendFunctionsLowering may have their own opinion about this
                    || context.isLambdaNonInlinable(lambdaClass)
                    ...
                    || capturedVariablesDeclaredInLoops.lambdaCapturesVariablesDeclaredInLoops(lambdaClass)
                ) {
                    buildFactoryFunction(ctorToFactoryMap, lambdaInfo)
                } else if (lambdaClass.fields.none()) {
                    // Optimization:
                    ...
                    // TODO: Only do this if a feature flag is enabled!
    ...
    private fun inlineLambdaBody(
        ...
        outerReceiverMapping: Map<IrFieldSymbol, IrGetField> = emptyMap() // TODO: Find a way to use a single map?
    ): IrBlockBody {
